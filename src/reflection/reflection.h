...
struct Type FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
         if constexpr (Index == 0) return base_type();
    else if constexpr (Index == 1) return element();
    else if constexpr (Index == 2) return index();
    else if constexpr (Index == 3) return fixed_length();
    else if constexpr (Index == 4) return base_size();
    else if constexpr (Index == 5) return element_size();
...
struct EnumVal FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
    ...
    else if constexpr (Index == 2) return union_type();
    else if constexpr (Index == 3) return documentation();
    else if constexpr (Index == 4) return attributes();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct Enum FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
         if constexpr (Index == 0) return name();
    else if constexpr (Index == 1) return values();
    else if constexpr (Index == 2) return is_union();
    else if constexpr (Index == 3) return underlying_type();
    else if constexpr (Index == 4) return attributes();
    else if constexpr (Index == 5) return documentation();
    else if constexpr (Index == 6) return declaration_file();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct Field FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
    ...
    else if constexpr (Index == 2) return id();
    else if constexpr (Index == 3) return offset();
    else if constexpr (Index == 4) return default_integer();
    else if constexpr (Index == 5) return default_real();
    else if constexpr (Index == 6) return deprecated();
    else if constexpr (Index == 7) return required();
    else if constexpr (Index == 8) return key();
    else if constexpr (Index == 9) return attributes();
    else if constexpr (Index == 10) return documentation();
    else if constexpr (Index == 11) return optional();
    else if constexpr (Index == 12) return padding();
    else if constexpr (Index == 13) return offset64();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct Object FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
         if constexpr (Index == 0) return name();
    else if constexpr (Index == 1) return fields();
    else if constexpr (Index == 2) return is_struct();
    else if constexpr (Index == 3) return minalign();
    else if constexpr (Index == 4) return bytesize();
    else if constexpr (Index == 5) return attributes();
    else if constexpr (Index == 6) return documentation();
    else if constexpr (Index == 7) return declaration_file();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct RPCCall FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
         if constexpr (Index == 0) return name();
    else if constexpr (Index == 1) return request();
    else if constexpr (Index == 2) return response();
    else if constexpr (Index == 3) return attributes();
    else if constexpr (Index == 4) return documentation();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct Service FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
         if constexpr (Index == 0) return name();
    else if constexpr (Index == 1) return calls();
    else if constexpr (Index == 2) return attributes();
    else if constexpr (Index == 3) return documentation();
    else if constexpr (Index == 4) return declaration_file();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct SchemaFile FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  auto get_field() const {
    ...
    else if constexpr (Index == 1) return included_filenames();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct Schema FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  template<size_t Index>
  auto get_field() const {
         if constexpr (Index == 0) return objects();
    else if constexpr (Index == 1) return enums();
    else if constexpr (Index == 2) return file_ident();
    else if constexpr (Index == 3) return file_ext();
    else if constexpr (Index == 4) return root_table();
    else if constexpr (Index == 5) return services();
    else if constexpr (Index == 6) return advanced_features();
    else if constexpr (Index == 7) return fbs_files();
    else static_assert(Index != Index, "Invalid Field Index");
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
inline bool operator==(const EnumValT &lhs, const EnumValT &rhs) {
  return
      ...
      (lhs.attributes.size() == rhs.attributes.size() && std::equal(lhs.attributes.cbegin(), lhs.attributes.cend(), rhs.attributes.cbegin(), [](std::unique_ptr<reflection::KeyValueT> const &a, std::unique_ptr<reflection::KeyValueT> const &b) { return (a == b) || (a && b && *a == *b); }));
}
...
inline bool operator==(const EnumT &lhs, const EnumT &rhs) {
  return
      (lhs.name == rhs.name) &&
      (lhs.values.size() == rhs.values.size() && std::equal(lhs.values.cbegin(), lhs.values.cend(), rhs.values.cbegin(), [](std::unique_ptr<reflection::EnumValT> const &a, std::unique_ptr<reflection::EnumValT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
...
inline bool operator==(const ObjectT &lhs, const ObjectT &rhs) {
  return
      (lhs.name == rhs.name) &&
      (lhs.fields.size() == rhs.fields.size() && std::equal(lhs.fields.cbegin(), lhs.fields.cend(), rhs.fields.cbegin(), [](std::unique_ptr<reflection::FieldT> const &a, std::unique_ptr<reflection::FieldT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
...
inline bool operator==(const ServiceT &lhs, const ServiceT &rhs) {
  return
      (lhs.name == rhs.name) &&
      (lhs.calls.size() == rhs.calls.size() && std::equal(lhs.calls.cbegin(), lhs.calls.cend(), rhs.calls.cbegin(), [](std::unique_ptr<reflection::RPCCallT> const &a, std::unique_ptr<reflection::RPCCallT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
      (lhs.attributes.size() == rhs.attributes.size() && std::equal(lhs.attributes.cbegin(), lhs.attributes.cend(), rhs.attributes.cbegin(), [](std::unique_ptr<reflection::KeyValueT> const &a, std::unique_ptr<reflection::KeyValueT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
      (lhs.documentation == rhs.documentation) &&
      (lhs.declaration_file == rhs.declaration_file);
}
...
inline bool operator==(const SchemaT &lhs, const SchemaT &rhs) {
  return
      (lhs.objects.size() == rhs.objects.size() && std::equal(lhs.objects.cbegin(), lhs.objects.cend(), rhs.objects.cbegin(), [](std::unique_ptr<reflection::ObjectT> const &a, std::unique_ptr<reflection::ObjectT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
      (lhs.enums.size() == rhs.enums.size() && std::equal(lhs.enums.cbegin(), lhs.enums.cend(), rhs.enums.cbegin(), [](std::unique_ptr<reflection::EnumT> const &a, std::unique_ptr<reflection::EnumT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
      ...
      (lhs.services.size() == rhs.services.size() && std::equal(lhs.services.cbegin(), lhs.services.cend(), rhs.services.cbegin(), [](std::unique_ptr<reflection::ServiceT> const &a, std::unique_ptr<reflection::ServiceT> const &b) { return (a == b) || (a && b && *a == *b); })) &&
      (lhs.advanced_features == rhs.advanced_features) &&
      (lhs.fbs_files.size() == rhs.fbs_files.size() && std::equal(lhs.fbs_files.cbegin(), lhs.fbs_files.cend(), rhs.fbs_files.cbegin(), [](std::unique_ptr<reflection::SchemaFileT> const &a, std::unique_ptr<reflection::SchemaFileT> const &b) { return (a == b) || (a && b && *a == *b); }));
}
